"""CycleGAN v2: Sweep all epoch checkpoints and evaluate."""
...
# Find all v2 epoch checkpoints
epochs = sorted([
    int(f.replace('cyclegan_v2_epoch_', '').replace('.pth', ''))
    for f in os.listdir(CHECKPOINT_DIR)
    if f.startswith('cyclegan_v2_epoch_') and f.endswith('.pth')
])
...
for epoch in epochs:
    ckpt_path = os.path.join(CHECKPOINT_DIR, f'cyclegan_v2_epoch_{epoch}.pth')
