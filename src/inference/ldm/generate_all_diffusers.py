...
checkpoint_path = 'checkpoints/ldm/best_model_v6_tightcrop.pt'
    ...
    try:
        ...
        with torch.no_grad():
            generated = model.translate(image, target_labels=torch.tensor([1], device=DEVICE),
                                         strength=0.85, num_steps=50, use_ema=True, guidance_scale=2.5)
    ...
    try:
        ...
        with torch.no_grad():
            generated = model.translate(image, target_labels=torch.tensor([0], device=DEVICE),
                                         strength=0.75, num_steps=50, use_ema=True, guidance_scale=2.0)
        save_image(generated.cpu(), out_path)
        del image, generated
        torch.cuda.empty_cache()
    except Exception as e:
    ...
    try:
        ...
        with torch.no_grad():
            generated = model.sample(num_samples=1, device=DEVICE, labels=label,
                                      use_ema=True, guidance_scale=2.0)
        save_image(generated.cpu(), out_path)
        del generated
        torch.cuda.empty_cache()
    except Exception as e:
