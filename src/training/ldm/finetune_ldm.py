    ...
    for epoch in range(START_EPOCH, NUM_EPOCHS):
        ...
        for images, labels in pbar:
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=USE_BFLOAT16):
                loss = model(images, labels=labels, step_count=train_steps)
            ...
            if accumulation_counter % GRADIENT_ACCUMULATION_STEPS == 0:
                ...
                torch.nn.utils.clip_grad_norm_(model.vae.decoder.parameters(), MAX_GRAD_NORM)
                optimizer.step()
                optimizer.zero_grad()
                model.update_ema()  
        ...
        if (epoch + 1) % VAL_FREQ == 0:
            ...
            with torch.no_grad():
                for images, labels in tqdm(val_dataloader, desc=f"Epoch {epoch+1}/{NUM_EPOCHS} [Val]"):
                    ...
                    labels = labels.to(DEVICE, non_blocking=True)
                    with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=USE_BFLOAT16):
                        loss = model(images, labels=labels, step_count=val_steps)
